...
def test_supports_timestamps():
    ...
    assert bag_storage.supports_timestamps("mcap") is False
...
def test_mcap_reader_is_an_unimplemented_extension_point(tmp_path):
    reader = bag_storage.McapReader()
    assert reader.supported is False
    with pytest.raises(bag_storage.BagStorageUnsupported):
        reader.topic_timestamps(tmp_path, [])
